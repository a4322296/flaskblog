...
class User(UserMixin, db.Model):
    ...
    def generate_confirmation_token(self, expiration=3600):
        # itsdangerous 提供了多种生成令牌的方法。其中， TimedJSONWebSignatureSerializer 类生成具有过期时间的 JSON Web 签名（JSON Web Signatures，JWS）。这个类的构造函数接收的参数是一个密钥，在 Flask 程序中可使用 SECRET
    ...
    # 点赞的方法
    def agree(self, post):
        print(post.agreements_num)
        # if not self.is_agreeing(answer):
        f = Agreement(user_id=self.id, post_id=post.id)
        post.agreements_num = post.agreements_num + 1
        db.session.add(f)
        db.session.add(post)

    def unagree(self, post):
        f = Agreement.query.filter_by(user_id=self.id).filter_by(post_id=post.id).first()
        post.agreements_num = post.agreements_num - 1
        db.session.delete(f)
        db.session.add(post)

    def is_agreeing(self, post):

        return Agreement.query.filter_by(user_id=self.id).filter_by(post_id=post.id).first() is not None
...
class Post(db.Model):
    ...
    tags = db.relationship('Tag', secondary=post_tags_table, backref=db.backref('posts',
                                                                                lazy='dynamic'), lazy='dynamic')
    post = db.relationship('Agreement', backref='post', lazy='dynamic')
